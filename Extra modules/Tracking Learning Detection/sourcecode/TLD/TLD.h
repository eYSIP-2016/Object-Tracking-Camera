...
struct BoundingBox : public cv::Rect
{
    ...
    BoundingBox(const cv::Rect &r) : cv::Rect(r)
    {
...
public:
    ...
    void generatePositiveData(const cv::Mat &frame, const int &num_warps);
    void generateNegativeData(const cv::Mat &frame);
    void processFrame(const cv::Mat &img1,
                      const cv::Mat &img2,
                      BoundingBox &bbnext,
                      bool &lastboxfound);
    void track(const cv::Mat &img1, const cv::Mat &img2);
    void detect(const cv::Mat &frame);
    void clusterConf(const std::vector<BoundingBox> &dbb,
                     const std::vector<float> &dconf,
    ...
    void getOverlappingBoxes(const cv::Rect &box1, const int &num_closest);
    void getBBHull();
    ...
    void bbPredict(const std::vector<cv::Point2f> &points1,
