...
float FerNNClassifier::measure_forest(const vector<int> &fern)
{
    float votes = 0;
    for (int i = 0; i < nstructs; i++) {
...
void FerNNClassifier::update(const vector<int> &fern, const int &C, const int &N) {
    int idx;
    for (int i = 0; i < nstructs; i++) {
        idx = fern[i];
...
void FerNNClassifier::trainF(const vector<pair<vector<int>,int> > &ferns, const int &resample)
{
